...
void Fastkat::load(Ogre::SceneNode* root_scene_node, Ogre::SceneManager* scene) {
  ...
  for(int entity_id = 0; entity_id < number_of_entities; ++entity_id)
  {
    ...
    float position_offset = 1+ entity_id * 2 - number_of_entities;
...
bool Fastkat::update(float delta_time, Ogre::SceneNode* camera_node, OIS::Keyboard* keyboard, OIS::Mouse* mouse, float window_width, float window_height) {
