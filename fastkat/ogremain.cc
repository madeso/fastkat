...
// use the gui instead?
std::auto_ptr<Ogre::Root> InitOgre(Ogre::String window_title, Ogre::RenderWindow** window)
{
  ...
  root.reset(new Ogre::Root(config_file_name, plugins_file_name, log_file_name));
  {
    ...
    {
      for(Strings::iterator plugin_iterator = plugin_names.begin();plugin_iterator != plugin_names.end(); plugin_iterator++)
      {
...

  {
    ...
    root->initialise(create_window_automatically, window_title, custom_capacities);
  }
  ...
  params["FSAA"] = "0"; 
  params["vsync"] = "true";
  *window = root->createRenderWindow(window_title, window_width, window_height, is_fullscreen, &params);
...
void OgreMain()
{
  ...
  std::auto_ptr<Ogre::Root> root = InitOgre("fastkat", &window);
	...
	OIS::Keyboard* keyboard = NULL;
	{
		...
		special_parameters.insert(std::make_pair(std::string("WINDOW"), window_handle_as_string));
		...
		keyboard = static_cast<OIS::Keyboard*>(input_manager->createInputObject(OIS::OISKeyboard, buffered_keys));

		mouse = static_cast<OIS::Mouse*>(input_manager->createInputObject( OIS::OISMouse, buffered_mouse));
	...
	Ogre::SceneManager* scene = root->createSceneManager(Ogre::ST_GENERIC, "MyFirstSceneManager");

	Ogre::SceneNode* root_scene_node = scene->getRootSceneNode();
  Ogre::Camera* camera = scene->createCamera("MyFirstCamera");
  Ogre::SceneNode* camera_node = root_scene_node->createChildSceneNode("MyFirstCameraNode");
	...
	unsigned short main_viewport_zorder = 100;
	Ogre::Viewport * viewport = window->addViewport(camera, main_viewport_zorder, viewport_left, viewport_top, viewport_width, viewport_height);
  ...
  Fastkat fastkat;
	...
	Ogre::String resource_group_name = "resourcegroup";
	{
		...
		resource_group_manager.addResourceLocation(directory_to_load, "FileSystem", resource_group_name, is_recursive);
	...
	unsigned long last_time_ms = timer->getMilliseconds();
	...
	{
		{
			unsigned long current_time_ms = timer->getMilliseconds();
			...
			delta_time = 0.001f * float(delta_time_ms);
		}

		keyboard->capture();
    mouse->capture();

		if( fastkat.update(delta_time, camera_node, keyboard, mouse, window->getWidth(), window->getHeight()) == false) {
      break;
    }
